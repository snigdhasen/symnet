...
parser.add_argument('--flag-type',type=str, required=True,help='Adaptive/constant LR')
...
def main():
    ...
    if task=='regression':
        model=RegressionModel(args.dataset, n_classes=num_classes, label_column=labels, task=task, header=has_header,
                             activation=activation, bs=bs, train_size=train_split, epochs=n_epochs, balance=False,optimizer='sgd',flag_type=flag_type)
